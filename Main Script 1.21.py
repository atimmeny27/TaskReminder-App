...
def setup_database():
    ...
    # 2. Now safely alter the table to add notify_at
    try:
    ...
    #  4. Only insert Tropical if NO themes exist
...
class DraggableFolder(RelativeLayout):
    def __init__(self, folder_name, icon_path, app_ref, **kwargs):
        super().__init__(size_hint_y=None, height=Window.height * 0.075, **kwargs)  # 8% of window height
...
class CircleButton(Button):
    def __init__(self, app_ref=None, **kwargs):
        ...
        with self.canvas.before:
            ...
            stroke_width = Window.width * 0.002  # ~0.2% of window width
    ...
    def update_graphics(self, *args):
        ...
        self.stroke_circle.circle = (
            ...
            min(self.width, self.height) / 2 - Window.width * 0.001  # ~2 pixels on a 1000px wide screen
        )
...
class CalendarCell(Button):
    def __init__(self, day, **kwargs):
        ...
        self.size = (100, 70)
        self.background_normal = ''
        self.background_color = (0.91, 0.902, 0.875, 1)  # creme
        self.color = (0, 0, 0, 1)
...
class CalendarGrid(GridLayout):
    def __init__(self, **kwargs):
        super().__init__(cols=7, spacing=2, padding=10, **kwargs)
        self.size_hint = (None, None)
        self.size = (700, 500)
        self.build_calendar()
    ...
    def build_calendar(self):
        ...
        for _ in range(first_day):
            self.add_widget(Label(size_hint=(None, None), size=(100, 70)))

        # Dates
        for day in range(1, days_in_month + 1):
            self.add_widget(CalendarCell(day))

        # Create but DON'T add label here
    ...
    def update_time_label(self, *args):
        ...
        self.time_label.text = f"[b]Current Time: {now}[/b]"
...
class DraggableReminder(BoxLayout):
    def __init__(self, reminder_text, urgency_level, folder_name, app_ref, notify_at=None, **kwargs):
        ...
        # Wrap the button in a FloatLayout for positioning control
        ...
        if urgency_level == 2:
            self.urgency_image = Image(
                source='Images for App/lvl1.png',  # <-- use correct path here
                size_hint=(None, 1),
        ...
        if self.notify_at:
            try:
                dt = datetime.fromisoformat(self.notify_at)
                ...
                # Label for datetime
                notify_label = Label(
                    text=f"[size=25][b][i]{formatted_time}[/i][/size]",
                    markup=True,
                    halign="left",
                    valign="middle",
                    size_hint_y=None,
                ...
                # Also apply red color to the main reminder text
                if is_overdue:
                    self.text_btn.color = text_color
                notify_label.bind(size=notify_label.setter('text_size'))
                info_box.add_widget(notify_label)
            except ValueError:
                pass  # handle if notify_at is malformed
    ...
    def open_calendar_popup(self, instance):
        ...
        for month in month_names:
            ...
            with month_label.canvas.before:
                Color(0.91, 0.902, 0.875, 1)  # Creme background
                Rectangle(pos=month_label.pos, size=month_label.size)
                month_label.bind(pos=lambda inst, val: setattr(inst.canvas.before.children[-1], 'pos', val))
                month_label.bind(size=lambda inst, val: setattr(inst.canvas.before.children[-1], 'size', val))

            month_sidebar.add_widget(month_label)
        ...
        calendar.time_label.font_size = 40

        weekday_bg = FloatLayout(
            size_hint=(None, None),
            size=(715, 50),
            pos_hint={"center_x": 0.564, "center_y": 0.73}
        )

        # Background rectangle
        with weekday_bg.canvas.before:
            Color(0.91, 0.902, 0.875, 1)  # Creme
        ...
        # Keep border synced on resize/move
        ...
        with time_box.canvas.before:
            Color(0.91, 0.902, 0.875, 1)  # Creme color
        ...
        calendar.time_label.pos_hint = {"center_x": 0.5, "center_y": 0.5}
        time_box.add_widget(calendar.time_label)
        layout.add_widget(time_box)

        Clock.schedule_interval(calendar.update_time_label, 1)
...
class RemindersApp(App):
    ...
    def build(self):
        ...
        with self.root_layout.canvas.after:
            ...
            self.divider_line_left = Line(points=[0, 0, 0, 0], width=8)
    ...
    def mark_as_completed(self, reminder_text, current_folder):
        ...
        # Delete this reminder from every folder except Completed
        cursor.execute("""
            DELETE FROM reminders
            WHERE text = ? AND folder_name != 'Completed'
        """, (reminder_text,))

        # Insert into Completed if not already there
        cursor.execute("""
    ...
    def setup_ui(self):
        ...
        # Calculate mid-point between dividers
        left_divider_x = Window.width * 0.19
        right_divider_x = Window.width * 0.72
        center_x_normalized = ((left_divider_x + right_divider_x) / 2) / Window.width

        # Now create the label using that value
    ...
    def show_custom_theme_popup(self, instance=None):
        ...
        close_btn = Button(
            ...
            size_hint_x=0.1,
            background_normal='',
            background_color=(1, 0.2, 0.2, 1),  # red-ish button
            color=(1, 1, 1, 1),  # white text
            bold=True
        )
    ...
    def clean_color_picker_ui(self, color_picker):
        ...
        for child in color_picker.children:
            if isinstance(child, BoxLayout):
                ...
                # Find any "A", "Alpha", or "X" slider labels and remove them
                to_remove = []
        ...
        return target_box  # where we'll insert custom UI
    ...
    def load_reminders(self, folder_name):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT text, urgency_level, notify_at FROM reminders WHERE folder_name = ? ORDER BY reminder_order ASC",
            (folder_name_clean,))
        reminders = cursor.fetchall()
        conn.close()
        ...
        # Build widgets (include notify_at for display)
        for text, urgency, notify_at in reminders:
    ...
    def set_light_theme(self, instance=None):
        self.theme = {
            ...
            "secondary": (0.176, 0.176, 0.231, 1),  # black/blue
    ...
    def create_complete_button(self, reminder_text, folder_name):
        ...
        with btn.canvas.before:
            Color(*self.theme["secondary"])  #Uses the secondary theme
